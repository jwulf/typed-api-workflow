...
if mvn exec:java@analyze -Dexec.args="$PROJECT_ROOT/zeebe/gateway-rest/src/main/java/io/camunda/zeebe/gateway/rest/controller $PROJECT_ROOT/../rest-api.domain.yaml" -q; then
    echo "✅ All endpoints are consistent!"
    exit 0
else
    echo "❌ Inconsistencies found. Run './patch-openapi-domain.sh' to fix them automatically."
    exit 1
fi
