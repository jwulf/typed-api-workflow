...
describe('facade ergonomics', () => {
  ...
  it('exposes body-only overload accepting raw body', () => {
    const camunda = new CamundaClient({ config: { CAMUNDA_REST_ADDRESS: 'http://localhost:1234' }, fetch: vi.fn(async () => new Response(JSON.stringify({ ok: true }), { status: 200, headers: { 'Content-Type': 'application/json' } })) });
  ...
  it('exposes passthrough operation', () => {
    const camunda = new CamundaClient()
