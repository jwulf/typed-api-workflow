...
describe('facade ergonomics', () => {
  ...
  it('exposes body-only overload accepting raw body', () => {
    const camunda = new Camunda8({ config: { CAMUNDA_REST_ADDRESS: 'http://localhost:1234' }, fetch: vi.fn(async () => new Response(JSON.stringify({ ok: true }), { status: 200, headers: { 'Content-Type': 'application/json' } })) });
