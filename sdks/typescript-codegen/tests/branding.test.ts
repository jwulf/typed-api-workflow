...
describe('branding nominal incompatibility', () => {
  it('namespace create produces nominal objects not inter-assignable (compile-time)', () => {
    const raw = '123456';
  const pik: ProcessInstanceKey = CamundaKeys.ProcessInstanceKey.assumeExists(raw);
  const pdk: ProcessDefinitionKey = CamundaKeys.ProcessDefinitionKey.assumeExists(raw);
