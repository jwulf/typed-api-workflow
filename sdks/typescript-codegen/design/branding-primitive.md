...
## Factories
Each key namespace exposes `create`, `getValue`, `equals`, `isValid` retaining the prior API surface, but implementations are simplified:
```ts
export namespace ProcessInstanceKey {
  export function create(value: string): ProcessInstanceKey { /* validate */ return value as ProcessInstanceKey; }
