...
export function computeCoverage(records: FieldRecord[]): CoverageStats {
  ...
  const covered = leafs.filter(r => r.hasExample || r.inheritedExample || r.synthetic).length;
  const uncovered = total - covered;
  const coveragePercent = total === 0 ? 100 : +( (covered / total) * 100 ).toFixed(2);
  const origin: Record<string, { total: number; covered: number }> = {};
  let direct = 0, inherited = 0, synthetic = 0;
  for (const r of leafs) {
    origin[r.origin] ||= { total: 0, covered: 0 };
    origin[r.origin].total++;
    if (r.hasExample || r.inheritedExample || r.synthetic) {
      origin[r.origin].covered++;
      if (r.hasExample) direct++; 
      else if (r.inheritedExample) inherited++; 
      else if (r.synthetic) synthetic++;
    }
  }
  return {
    total,
    ...
    uncovered,
    coveragePercent,
    breakdown: { origin, inheritance: { direct, inherited }, synthetic }
  };
}
